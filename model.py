...
class CaptionDecoder(nn.Module):
    ...
    def generate_caption(
        self, hidden_state, vocab, max_length=50, method="beam", beam_size=3
    ):
        if method == "greedy":
    ...
    def _beam_search(self, hidden_state, vocab, max_length, beam_size):
        ...
        for _ in range(max_length):
            ...
            for seq, score, h_state, c_state in beams:
                ...
                lstm_out, (h_new, c_new) = self.lstm(
                    self.embedding(input_word), (h_state, c_state)
                )
        ...
        indices = [
            idx
            for idx in best_seq
            if idx not in [self.start_idx, self.end_idx, self.pad_idx]
        ]
...
class ImageCaptioningModel(nn.Module):
    ...
    def generate_caption(
        self, image_features, vocab, max_length=50, method="beam", beam_size=3
    ):
        self.eval()
        with torch.no_grad():
            ...
            return self.decoder.generate_caption(
                hidden_state, vocab, max_length, method, beam_size
            )
